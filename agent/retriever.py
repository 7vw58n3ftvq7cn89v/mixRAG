...
class Retriever:
    def __init__(self, agent_type, mode, embed_model_name, top_k = 5, max_encode_cell = 10000, db_dir = 'db/', verbose = False):
        ...
        self.local_model_path = "./local_models/"
        os.makedirs(db_dir, exist_ok=True)
        # mixRAG
        self.wikidata_schema_retriever = None
        self.wikidata_path = './data/wikidata/{qid}.json'
        ...
        if self.mode == 'bm25':
            ...
            # print('config:', config)
            self.embedder = OpenAIEmbeddings(**config)
        ...
        elif 'local' in self.embed_model_name:
