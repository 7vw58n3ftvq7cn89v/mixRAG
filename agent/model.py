...
# os.environ["OPENAI_API_KEY"] = "sk-tuhtuuoysbjmhqnmvuonlpbndfauxgisopueoexiduzvdplp"
...
class Model:
    def __init__(self, model_name, provider=None):
        ...
        self.provider = provider if provider else self.get_provider(model_name)
        self.context_limit = self.get_context_limit(model_name)
        self.local_model_path = os.environ['LOCAL_MODELS']
        # OpenAI models
        ...
        # SiliconFlow models
            ...
            local_model_path = os.environ['LOCAL_MODELS']
            try:
                self.tokenizer = AutoTokenizer.from_pretrained(os.path.join(local_model_path, model_name))
            ...
            local_model_path = os.environ['LOCAL_MODELS']
            try:
                self.tokenizer = AutoTokenizer.from_pretrained(os.path.join(local_model_path, "deepseek-ai/DeepSeek-V2.5")) # 暂时用DeepSeek-V2.5的tokenizer
            except Exception as e:
                print(f"Error loading local tokenizer: {e}")
    ...
    def get_context_limit(self, model_name):
        if model_name == 'gpt-4-0125-preview' or model_name == 'gpt-4-turbo-2024-04-09' or model_name == 'gpt-4o-mini-2024-07-18':
            ...
            return 128000
        elif 'deepseek' in model_name or 'Qwen' in model_name or 'qwen' in model_name:
            return 128000
        else:
    ...
    def get_token_count(self, prompt):
        if not prompt:
            return 0
        if self.provider == 'openai' or self.provider == 'deepseek' or self.provider == 'siliconflow' or self.provider == 'Qwen':
